...
function atLeast(user: User, role: Role): boolean {
...
export const handlers = [
  ...
  http.patch(`${BASE}/users/:id`, async ({ request, params }) => {
    ...
    const body = (await request.json()) as Partial<Pick<User, 'name' | 'email' | 'role' | 'status'>>
    ...
    if (body.email !== undefined) {
      ...
      } else if (users.some((u) => u.id !== user.id && u.email === body.email)) {
